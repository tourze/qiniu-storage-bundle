 ...
 * 七牛云存储空间天统计管理控制器
 *
 * @extends AbstractCrudController<BucketDayStatistic>
 */
#[AdminCrud(routePath: '/qiniu-storage/bucket-day-statistic', routeName: 'qiniu_storage_bucket_day_statistic')]
final class BucketDayStatisticCrudController extends AbstractCrudController
{
    public static function getEntityFqcn(): string
    {
        return BucketDayStatistic::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('存储空间天统计')
            ->setEntityLabelInPlural('存储空间天统计列表')
            ->setPageTitle('index', '存储空间天统计管理')
            ->setPageTitle('detail', fn (BucketDayStatistic $statistic) => sprintf('存储空间天统计详情: %s - %s', $statistic->getBucket()->getName(), $statistic->getTime()->format('Y-m-d')))
            ->setDefaultSort(['time' => 'DESC'])
            ->setSearchFields(['bucket.name'])
            ->setHelp('index', '查看七牛云存储空间的天统计数据')
        ;
    }
