 ...
 * 七牛云存储空间分钟统计管理控制器
 *
 * @extends AbstractCrudController<BucketMinuteStatistic>
 */
#[AdminCrud(routePath: '/qiniu-storage/bucket-minute-statistic', routeName: 'qiniu_storage_bucket_minute_statistic')]
final class BucketMinuteStatisticCrudController extends AbstractCrudController
{
    public static function getEntityFqcn(): string
    {
        return BucketMinuteStatistic::class;
    }

    public function configureCrud(Crud $crud): Crud
    {
        return $crud
            ->setEntityLabelInSingular('存储空间分钟统计')
            ->setEntityLabelInPlural('存储空间分钟统计列表')
            ->setPageTitle('index', '存储空间分钟统计管理')
            ->setPageTitle('detail', fn (BucketMinuteStatistic $statistic) => sprintf('存储空间分钟统计详情: %s - %s', $statistic->getBucket()->getName(), $statistic->getTime()->format('Y-m-d H:i')))
            ->setDefaultSort(['time' => 'DESC'])
            ->setSearchFields(['bucket.name'])
            ->setHelp('index', '查看七牛云存储空间的分钟统计数据')
        ;
    }
